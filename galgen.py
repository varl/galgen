...
if __name__ == '__main__':
    ...
    for image in images:
        ...
        path = os.path.join(directory, thumb)

        im.thumbnail(THUMB_SIZE, Image.ANTIALIAS)
        o = info.get('Orientation', -1)
        if o == 8:
            print('[FLIP] Rotate 90 deg cw.')
            im = im.rotate(90)
        im.save(path, "JPEG")
        thumbed.append((image[0], thumb, info))
        print('[DONE] Created thumbnail for "{}"..'.format(file))

    with open(os.path.join(directory, 'index.html'), 'w') as index:
        print('Write the document')
        index.write(build_doc(directory, thumbed))
